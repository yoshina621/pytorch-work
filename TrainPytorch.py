...
class TrainPytorch(object):
	...
	def makeReward(self, finishRound):
		if finishRound == 0:
			...
			elif self.AgentType == 2: # NormalType
				reward = abs(self.nonDelay.getCharacter(not self.player).getHp()) - self.lastHp
				reward +=  self.SubPoint - (abs(self.nonDelay.getCharacter(self.player).getHp()) - self.lastHp2)
				print(reward)
				return reward
		else:
	...
	def setLastHp(self):
		...
		elif self.AgentType == 2: # Normal Type
			self.lastHp = abs(self.nonDelay.getCharacter(not self.player).getHp())
			self.lastHp2 = abs(self.nonDelay.getCharacter(self.player).getHp())
	...
	def pushData(self, next_state, reward):
		self.memory.push((self.FloatTensor([self.state]), self.action, self.FloatTensor([next_state]), self.FloatTensor([reward])))
